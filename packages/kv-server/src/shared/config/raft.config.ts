...
export class RaftConfigFactory {
  static createRaftConfiguration(
    configService: ConfigService,
  ): RaftConfiguration {
    const nodeId = configService.get<string>(
      ENV_KEYS.RAFT_NODE_ID,
      NODE_DEFAULTS.NODE_ID,
    );
    const clusterId = configService.get<string>(
      ENV_KEYS.RAFT_CLUSTER_ID,
      NODE_DEFAULTS.CLUSTER_ID,
    );

    return {
      nodeId,
      clusterId,
      httpHost: configService.get<string>(
        ENV_KEYS.RAFT_HTTP_HOST,
        DEFAULT_CONFIG.SERVER.HOST,
      ),
      httpPort: configService.get<number>(
        ENV_KEYS.RAFT_HTTP_PORT,
        DEFAULT_CONFIG.RAFT.HTTP_PORT,
      ),
      electionTimeout: [
        configService.get<number>(
          ENV_KEYS.RAFT_ELECTION_MIN,
          DEFAULT_CONFIG.RAFT.ELECTION.MIN,
        ),
        configService.get<number>(
          ENV_KEYS.RAFT_ELECTION_MAX,
          DEFAULT_CONFIG.RAFT.ELECTION.MAX,
        ),
      ],
      ...
      voting: {
        enableWeighting: configService.get<boolean>(
          "RAFT_ENABLE_VOTE_WEIGHTING",
          false,
        ),
      ...
      metrics: {
        enablePrometheus: configService.get<boolean>(
          "RAFT_ENABLE_PROMETHEUS",
          true,
        ),
        enableInternal: configService.get<boolean>(
          "RAFT_ENABLE_INTERNAL_METRICS",
          true,
        ),
      ...
      logging: {
        level: configService.get<any>(
          "RAFT_LOG_LEVEL",
          DEFAULT_CONFIG.LOGGING.LEVEL,
        ),
        redactedFields: [...DEFAULT_CONFIG.LOGGING.REDACTED_FIELDS],
        enableStructured: configService.get<boolean>(
          "RAFT_ENABLE_STRUCTURED_LOGGING",
          true,
        ),
        enableConsole: configService.get<boolean>(
          "RAFT_ENABLE_CONSOLE_LOGGING",
          true,
        ),
        enableFile: configService.get<boolean>(
          "RAFT_ENABLE_FILE_LOGGING",
          false,
        ),
        filePath: configService.get<string>(
          "RAFT_LOG_FILE_PATH",
          DEFAULT_CONFIG.LOGGING.FILE_PATH,
        ),
      },
      network: {
      ...
      persistence: {
        enableSnapshots: configService.get<boolean>(
          "RAFT_ENABLE_SNAPSHOTS",
          true,
        ),
        ...
        dataDir: configService.get<string>(
          "RAFT_DATA_DIR",
          DEFAULT_CONFIG.RAFT.DATA_DIR,
        ),
